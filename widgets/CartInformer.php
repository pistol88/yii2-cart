...
class CartInformer extends \yii\base\Widget
{
    ...
    public function run()
    {
        $cartElementModel = yii::$app->cart;
        $this->text = str_replace(['{c}', '{p}'],
            ['<span class="pistol88-cart-count">'.$cartElementModel->getCount().'</span>', '<strong class="pistol88-cart-price">'.$cartElementModel->getCostFormatted().'</strong>'],
            $this->text
        );
